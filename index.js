...
for (letter of present) {
  ...
  if (actual != letter)
    console.error(`The map for '${letter}' is wrong! It evaluated to ${actual}.`);
}
...
let presentPerf = perf(present);
console.log(`There are ${present.length} mapped characters. Average code length is ${Math.round(presentPerf)}.`);
